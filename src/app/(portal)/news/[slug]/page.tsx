...
export default async function NewsDetailPage({ params }: PageProps) {
  ...
  const publishedDate = format(new Date(item.publishedAt), "d 'de' MMMM 'de' yyyy 'às' HH:mm", { locale: ptBR });

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      <Link href="/noticias" className="inline-flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-brand-600 dark:hover:text-brand-400 transition-colors mb-8 group">
      ...
      <article className="bg-white dark:bg-gray-900 rounded-2xl shadow-md border border-gray-100 dark:border-gray-800 overflow-hidden">
        {item.imageUrl && (
          <div className="relative h-64 sm:h-80 lg:h-96 bg-gray-100 dark:bg-gray-800">
            <Image src={item.imageUrl} alt={item.title} fill className="object-cover" priority sizes="(max-width: 896px) 100vw, 896px" />
          </div>
        )}
        <div className="p-6 sm:p-8 lg:p-10">
          <div className="flex flex-wrap items-center gap-3 mb-5">
            ...
            {item.isImportant && (
              <span className="inline-flex items-center gap-1 text-xs font-semibold px-3 py-1 rounded-full bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">
                <StarIcon className="w-3 h-3" /> Destaque
              </span>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-gray-400 pb-6 mb-6 border-b border-gray-100 dark:border-gray-800">
            ...
            <span className="flex items-center gap-1.5"><CalendarIcon className="w-4 h-4" />{publishedDate}</span>
          </div>

          <p className="text-lg sm:text-xl text-gray-600 dark:text-gray-300 leading-relaxed mb-6 font-medium">{item.summary}</p>

          {item.content && item.content !== item.summary && (
            <div className="prose prose-gray dark:prose-invert prose-lg max-w-none mb-8">
              <p className="text-gray-600 dark:text-gray-400 leading-relaxed whitespace-pre-line">{item.content}</p>
            </div>
          )}
          ...
          <div className="mt-8 p-5 bg-brand-50 dark:bg-brand-900/20 border border-brand-100 dark:border-brand-800/30 rounded-xl">
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Este é um resumo gerado a partir do feed RSS de <strong className="text-gray-800 dark:text-gray-200">{item.sourceName}</strong>.
            </p>
            <a href={item.sourceUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-500 hover:bg-brand-600 text-white font-semibold rounded-xl transition-colors shadow-md shadow-brand-500/20 text-sm">
              Ler notícia completa <ArrowTopRightIcon className="w-4 h-4" />
            </a>
          </div>
        </div>
      </article>
      ...
      <div className="mt-8 text-center">
        <Link href="/noticias" className="inline-flex items-center gap-2 text-sm font-medium text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 transition-colors">
          <ArrowLeftIcon className="w-4 h-4" /> Ver mais notícias
        </Link>
      </div>
    </div>
  );
